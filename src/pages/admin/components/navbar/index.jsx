...
const Navbar = (props) => {
  ...
  const getPageTitle = () => {
    if (location.pathname === '/admin/manager-product/add-product') {
      ...
      return 'Edit Member'
    } else if (location.pathname.startsWith('/admin/manager-board/add-board')) {
      return 'Add Board'
    } else if (location.pathname.startsWith('/admin/manager-board/edit-board')) {
      return 'Edit Board'
    } else {
      return ''
    }
    ...
    <nav className='mx-2 sticky top-4 z-40 flex flex-row flex-wrap items-center justify-between rounded-lg p-2 backdrop-blur-xl bg-white'>
      <div className='ml-[6px]'>
        <p className='shrink text-largerPrdName capitalize flex gap-1'>
          <Link to='#' className='font-bold text-[#3B3B3B] lg:text-largerPrdName text-normal'>
            {brandText}
          </Link>
          {getPageTitle() && (
            <>
              <img src={IconArrowRight} alt='icon' />
              <Link className='font-bold lg:text-largerPrdName text-normal text-[#3B3B3B]'>{getPageTitle()}</Link>
            </>
          )}
        </p>
      </div>
