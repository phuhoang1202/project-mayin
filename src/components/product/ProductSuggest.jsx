        ...
        <div className='lg:grid lg:grid-cols-3 gap-6 lg:pb-0 pb-4 lg:overflow-hidden overflow-x-auto flex'>
          {isLoading
            ...
            : recommendProducts.map((product, index) => {
                ...
                const hasType = product.promotions?.some((promotion) => promotion.type === 'recommend')
                const hasEarnPoint = product.promotions?.some((promotion) => promotion.type === 'earn_points')
                ...
                return (
                  ...
                  >
                    ...
                    <Link to={`/product/${product.id}`} className='flex justify-between'>
                      <div className='flex flex-col justify-between lg:w-[220px] py-1 w-44'>
                        <div className='flex flex-col gap-2'>
                          <div className='font-semibold lg:text-textPrd text-primaryPrdName'>
                            <Tooltip title={product.productName}>
                              <span className='block truncate w-full text-[#3B3B3B]'>{product?.productName}</span>
                            </Tooltip>
                          </div>

                          <div className='text-normal text-[#8C8C8C] block truncate w-full'>
                        ...
                        <div className='mb-1 flex gap-1'>
                          {hasType && (
                            <span className='text-[#2DC033] font-semibold text-min min-w-[64px] h-6 bg-[#EFEFEF] flex justify-center items-center'>
                              {t('titleSuggest')}
                            </span>
                          )}
                        </div>
