...
const ManagerProduct = () => {
  ...
  const fetchDataProduct = async (pageNumber = 0, pageSize = 6, translatedText = searchTextTranslate) => {
    try {
      setLoading(true)
      const bodyPayload = {
        pageNumber,
    ...
    },
    {
      ...
      render: (promotions, _) => {
        return promotions?.length > 0 ? (
          <div style={{ maxWidth: '150px', whiteSpace: 'normal', wordWrap: 'break-word' }}>
            {promotions.map((promotion, index) => (
              <Tag color={getColor(promotion.type)} className='m-1' key={index}>
                {promotion.name}
              </Tag>
            ))}
          </div>
  ...
  return (
    <div>
      <Card extra={'w-full h-[87vh] overflow-y-auto p-4'}>
        <div>
          <div className='flex lg:flex-row flex-col justify-between m-6 gap-4'>
          ...
          /> */}
            ...
            <div className='flex items-center flex-row gap-4 lg:mt-0 mt-4'>
              <div className='flex flex-col md:flex-row items-center w-full md:w-auto relative flex-1'>
                <Input
                  placeholder='검색 프로모션'
