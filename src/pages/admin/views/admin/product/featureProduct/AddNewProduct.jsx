...
import { Form, Input, Image, Upload, Radio, Space, TreeSelect, Select, message, Button } from 'antd'
...
import IconDecrease from '@assets/images/IconDecrease.svg'
...
export default function AddNewProduct() {
  ...
  const handleEmailChange = (index, value) => {
    setDataInput((prev) => {
      const updatedEmails = [...prev.emailConfirmPayment]
      updatedEmails[index] = value
      return { ...prev, emailConfirmPayment: updatedEmails }
    })
  }

  const handleRemoveEmail = (index, remove) => {
    setDataInput((prev) => {
      const updatedEmails = [...prev.emailConfirmPayment]
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true)
      dataInput.emailConfirmPayment = dataInput.emailConfirmPayment.toString()
          ...
          <div>
            <Form form={form} onFinish={handleSubmit}>
              <div className='flex flex-col gap-2'>
                ...
                <div className='flex lg:flex-row flex-col lg:items-center lg:gap-6 gap-1'>
                  {/* Category */}
                  <div className='flex flex-col gap-1 flex-1'>
                    <label className='text-[#3B3B3B] font-medium text-normal flex items-start'>
                ...
                <div className='flex lg:flex-row flex-col lg:gap-6 gap-1'>
                  <div className='flex flex-col gap-1 flex-1'>
                    <label className='text-[#3B3B3B] font-medium text-normal flex items-start'>
                      Channel <img src={IconAsterisk} alt='icon' />
                    </label>
                    <Form.Item
                      name='platformType'
                    ...
                    >
                      <Select
                        defaultValue='링고'
                        className='h-11'
                        onChange={(value) => setDataInput({ ...dataInput, platformType: value })}
                        options={[
                          {
                            value: 'g_party',
                            label: '링고',
                  ...
                  <div className='flex flex-col gap-1 flex-1'>
                    <label className='text-[#3B3B3B] font-medium text-normal flex items-start'>
                      Provider Email <img src={IconAsterisk} alt='icon' />
                    </label>
                    <Form.List name='email'>
                      {(fields, { add, remove }) => (
                        <>
                          <Form.Item>
                            <Button type='dashed' className='h-11' onClick={() => add()} block icon={<PlusOutlined />}>
                              Add field
                            </Button>
                          </Form.Item>

                          {fields.map(({ key, name, ...restField }) => {
                            const allEmails = fields.map(
                              (field) => form.getFieldValue(['email', field.name, 'email']) || '',
                            )
                            return (
                              <div className='flex items-center gap-4' key={key}>
                                    ...
                                    },
                                    {
                                      validator: (_, value) => {
                                        if (
                                          ...
                                          allEmails.filter(
                                            (email, i) =>
                                              email === value && i !== fields.findIndex((f) => f.key === key),
                                          ).length > 0
                                        ) {
                                          return Promise.reject(
                                            'Duplicate email detected! Please enter a unique email.',
                                          )
                                        }
                                        return Promise.resolve()
                                      },
                                ...
                                <div className='h-11'>
                                  <button
                                    ...
                                    onClick={() => handleRemoveEmail(name, remove)}
                                  >
                                    <img src={IconDecrease} alt='icon' />
                                  </button>
                                </div>
                              </div>
                            )
