...
export default function UpdateQuestion() {
  ...
  const fetchQuestionId = async () => {
    try {
      if (id) {
        ...
        setTextReply(questionData.messages[1].content || '')
      }
    } catch (error) {
  ...
  const handleEditMessage = async () => {
    try {
      ...
      const payload = {
        content: textReply,
        consultationId: dataQuestion.id,
        id: dataQuestion.messages[1].id,
        seen: true,
      }

      await question.editMessage(payload)
      Toast.success('Edit message successful !')
      navigate('/admin/manager-question')
    } catch (error) {
      ...
      <Card extra={'w-full h-[87vh] overflow-y-auto p-4'}>
        ...
        <div className='lg:mx-32 md:mx-20 sm:mx-14 mx-10'>
          <div className='flex items-center justify-between'>
            <h3 className='font-bold text-largerPrdName'>Edit Question</h3>
              ...
              <div className='mt-2'>
                ...
                >
                  ...
                  <div className='flex items-center mt-4 justify-end gap-4'>
                    ...
                    <button
                      htmlType='submit'
                      className='h-11 min-w-56 text-white bg-[#5B4DFB] rounded-lg font-semibold'
                    >
                      Update
                    </button>
                  </div>
