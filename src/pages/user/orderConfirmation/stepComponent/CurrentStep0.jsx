    ...
    <div className='lg:w-[843px]'>
      ...
      {cartData && cartData.length > 0 ? (
        cartData.map((store, cartIndex) => {
          ...
          const hasEarnPoints = product?.promotions?.some((promotion) => promotion.type == 'earn_points')

          let totalEarnPoints = 0
          if (store.cartItemAttributeOptions && store.cartItemAttributeOptions.length > 0) {
            store.cartItemAttributeOptions.forEach((item) => {
              const price = item.productSku.price
              const quantityOrder = item.quantityOrder
              const subtotal = price * quantityOrder

              totalEarnPoints += subtotal
            })
          }
                  ...
                  <div className='flex rounded-lg lg:gap-5 gap-3 relative lg:mt-0 mt-3'>
                    ...
                    <div>
                      <div className='flex flex-col justify-between gap-2 py-1 lg:w-[531px] w-full h-full'>
                                  ...
                                  <div key={e.id}>
                                    <div>
                                      <div className='flex items-center gap-2'>
                                        <div>
                                          <span className='font-bold lg:text-primaryPrdName'>
                                        ...
                                        <div className='w-[48px] h-[22px] bg-[#2DC033] text-white text-small font-medium rounded-full flex justify-center items-center'>
                                          -{e.discountPercent}%
                                        </div>
                                      </div>
                        ...
                        {hasEarnPoints ? (
                          <div className='font-medium text-min text-[#F14646]'>
                            {t('plus')}{' '}
                            {store?.cartItemAttributeOptions && store?.cartItemAttributeOptions.length > 0
                              ? totalEarnPoints * 0.01
                              : store.product.price * store.quantity * 0.01}{' '}
                            {t('points')}
                          </div>
                        ) : (
                          <div></div>
      ...
      <div className='flex justify-center items-center lg:mt-[88px] mt-8'>
        ...
        >
          {t('next')} <img src={IconChevronRight} alt='icon' />
        </button>
      </div>
